...
$(function(){
    ...
    $("#searchBtn").click(function(){
		...
		location.href="${pageContext.request.contextPath}/menu05_00${pageMaker.makeQuery(1)}&searchType="+searchType+"&keyword="+keyword;
	});
							...
							<c:if test="${fn:length(topList) != 0}">
								<c:forEach var="item" items="${topList}">
									<tr class="noticeTop">
										...
										<td class="subject">
											<a href="${pageContext.request.contextPath}/menu05_00read${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}">${item.title}</a>
										</td>
						...
						<ul class="full">
							<c:if test="${pageMaker.cri.page == 1}">
								...
								<c:if test="${fn:length(topList) != 0}">
									<c:forEach var="item" items="${topList}">
										<li>
											<a href="${pageContext.request.contextPath}/menu05_00read${pageMaker.makeSearch(pageMaker.cri.page)}&no=${item.no}">
												<b> ${item.title}</b>
